...
var url = window.location.href;url = url.split("/");url = url[0] + "//" + url[2];url = url.slice(0, url.length - 4);url = url+"9999/slearn";var questionxmlurl = url + "/cont/"+ session + xmlname;var mat = url+ "/mat/"+ session + "/";
...
function loadQuestionsToMemory () {
	...
	$(xml).find('Question').each(function () {
		...
		option3[count] = mat + xml.find("QuestionImage")[count].nextElementSibling.children[2].firstChild.data;
		option4[count] =mat +  xml.find("QuestionImage")[count].nextElementSibling.children[3].firstChild.data;
...
function displayQuestions(count) {
	if ((currentquestionnumber+1) > questiontext.length) {
		...
		for(var j=0; j < optionArray.length; j++){
			...
			$("#option" + (j + 1) + "image").html("<div class='radio'><label> <span class='pl-5'><input type='radio' name='optradio' class='radioButton' id='option' value='" + optionArray[mcqshuffledquestionnumber[j]].optionid + "'></span><span><img src='" + optionArray[mcqshuffledquestionnumber[j]].optionvalue + "' style='width:80%;'/></span></label></div>");
		}
...
function checkGameStatus() {
	var wornganswer = countElement(0,answermarks); var trueanswer = countElement(1,answermarks);
