...
function loadQuestionsToMemory() {
    ...
    $(xml).find('Question').each(function () {
        ...
        question[count] = mat + xml.find("QuestionVideo")[count].firstChild.data;
        option1[count] = mat + xml.find("QuestionVideo")[count].nextElementSibling.children[0].firstChild.data;
        option2[count] = mat + xml.find("QuestionVideo")[count].nextElementSibling.children[1].firstChild.data;
        option3[count] = mat + xml.find("QuestionVideo")[count].nextElementSibling.children[2].firstChild.data;
        option4[count] = mat + xml.find("QuestionVideo")[count].nextElementSibling.children[3].firstChild.data;
...
function displayQuestions(count) {
    if ((currentquestionnumber + 1) > questiontext.length) {
        ...
        $("#questionvideo").html("<video width='100%' controls><source src='" + question[shuffledQuestionNumber[count]] + "' type='video/mp4'></video>");

        for (var j = 0; j < optionArray.length; j++) {
            //$("#op"+(j+1)).html("<center>"+optionArray[mcqshuffledquestionnumber[j]].optionid+"</center>");
            $("#option" + (j + 1) + "image").html("<div class='radio'><label> <span><input type='radio' name='optradio' class='radioButton' id='option' value='" + optionArray[mcqshuffledquestionnumber[j]].optionid + "'></span><span><img src='" + optionArray[mcqshuffledquestionnumber[j]].optionvalue + "' style='width:80%;'/></span></label></div>");
        }
