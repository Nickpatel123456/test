...
function loadQuestionsToMemory() {
    ...
    $(xml).find('Question').each(function () {
        ...
        option1[count] = mat + xml.find("QuestionText")[count].nextElementSibling.children[0].firstChild.data;
        option2[count] = mat + xml.find("QuestionText")[count].nextElementSibling.children[1].firstChild.data;
        option3[count] = mat + xml.find("QuestionText")[count].nextElementSibling.children[2].firstChild.data;
        option4[count] = mat + xml.find("QuestionText")[count].nextElementSibling.children[3].firstChild.data;
...
function displayQuestions(count) {
    if ((currentquestionnumber + 1) > question.length) {
        ...
        $("#questiontext").html("<span onclick='togglePlay(" + quesionId[shuffledQuestionNumber[count]] + ")'><img id='audioImg' src='/apps/slearn/slearntemplates/assets/pause.png' width='40'></span><span class='pl-5'> [" + (currentquestionnumber + 1) + "] " + question[shuffledQuestionNumber[count]] + "</span>");
        $("#imageoptiondiv").html("");

        for (var j = 0; j < optionArray.length; j++) {
            $("#imageoptiondiv").append(
                '<div class="col-md-3 col-xs-3">' +
                '<div class="radio"><span class="pl-5"><label><span class="span"><input class="radioButton" type="radio" name="optradio" id="optradio" value="' + optionArray[mcqshuffledquestionnumber[j]].optionid + '"></span><span class="span pl-10"><img class="optionImage img-responsive img-thumbnail" src="' + optionArray[mcqshuffledquestionnumber[j]].optionvalue + '"></span></label></div>'
                + '</div>'
            );
        }
