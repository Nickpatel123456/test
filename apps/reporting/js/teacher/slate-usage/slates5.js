angular.module('sledstudio')
    .controller('TeacherSlate5ReportController', function ($scope, ajaxCallsFactory, FromdateTodateFactory, dashboardServices, dataModalServices, tableOptionService) {
        ...
        $scope.getsubjectdata5 = function () {
            var standard_id = $scope.teacher_std_analysis5;
        ...
        $scope.getDivisionData5 = function () {
            ajaxCallsFactory.getCall(school_config)
                .then(function (response) {
                    var divId = response.data.standard_division_map[$scope.teacher_std_analysis5];
        ...
        $scope.loadslate5report = function () {
            ...
            var subjectid = $scope.teacher_sub_analysis5;
            var standard_id = $scope.teacher_std_analysis5;
            var divisionid = $scope.teacher_div_analysis5;
            ...
            ajaxCallsFactory.postCall(urlPath, filterdata)
                .then(function (response) {
                    var teacherusagedataid = response.data.data;

                    var urlPath1 = dashboardServices.teacherDetails(school_id);
                    ajaxCallsFactory.getCall(urlPath1)
                        .then(function (response1) {
                            var teachertDetailsdata = response1.data.data;

                            $scope.teacherusagedata = [];

                            angular.forEach(teacherusagedataid, function (value, key) {
                                this.push({
                                    name: teachertDetailsdata[key].user_detail.first_name + " " + teachertDetailsdata[key].user_detail.last_name,
                            ...
                            if ($scope.teacherusagedata.length == 0) {
                                $scope.showslate5table = false;
