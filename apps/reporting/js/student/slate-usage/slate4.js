...
.controller('Slate4ReportController',function ($scope, ajaxCallsFactory, FromdateTodateFactory, dashboardServices, dataModalServices, barchartcolorServices, convertHourServices) {
			...
			.then(function (response1){
				...
				.then(function (response2){
					...
					angular.forEach(subjectidlist, function(value, key) {
						...
						if(studentsubjectusagedata[value] == undefined){
							standarddata.push(0);
						}else{
							var hourconvert = convertHourServices.convertDurationToHour(studentsubjectusagedata[value].duration);
							standarddata.push(hourconvert);
						}
					});
					
					$scope.data4.push(standarddata);
